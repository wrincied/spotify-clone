...
export class MusicStoreService {
    ...
    // 📦 DATA STATE
    // ==========================================
    private albumsSubject = new BehaviorSubject<
        AlbumInterface[]
    >([]);
    ...
    public currentTrackSubject =
        new BehaviorSubject<SongInterface | null>(null);
    public currentTrack$ =
        this.currentTrackSubject.asObservable();

    private currentCoverSubject =
        new BehaviorSubject<string>('');
    public currentCover$ =
        this.currentCoverSubject.asObservable();
    ...
    private durationSubject = new BehaviorSubject<number>(
        0,
    );
    public duration$ = this.durationSubject.asObservable();

    private isBufferingSubject =
        new BehaviorSubject<boolean>(false);
    public isBuffering$ =
        this.isBufferingSubject.asObservable();

    private isLoopingSubject = new BehaviorSubject<boolean>(
        false,
    );
    public isLooping$ =
        this.isLoopingSubject.asObservable();
    ...
    playOrPause(
        ...
        if (
            current &&
            String(current.id) === String(song.id)
        ) {
            this.togglePlay();
        } else {
    ...
    private loadCurrentTrack() {
        ...
        let coverUrl: string =
            song.thumbnail ||
            this.currentContextCover ||
            '';
        if (coverUrl && !coverUrl.startsWith('http')) {
            const cleanPath = coverUrl.startsWith('/')
                ? coverUrl.slice(1)
                : coverUrl;
