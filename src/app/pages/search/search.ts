...
export class SearchComponent implements OnInit, OnChanges {
    ...
    private runFilter(albums?: AlbumInterface[]): void {
        const list =
            albums ?? this.musicStore.currentAlbums;
        ...
        const matchingTracks = allTracks.filter((track) => {
            ...
            const artist = (
                track.artist || ''
            ).toLowerCase(); // Используем description как artist
            return (
                title.includes(term) ||
        ...
        const perfectMatch = matchingTracks.find(
        ...
        if (perfectMatch) {
            ...
            this.topTrack = null;
            this.filteredTracks = matchingTracks.slice(
                0,
                5,
            );
        }
